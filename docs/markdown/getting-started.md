...
=== "json"
    ...
    ```bash
    db2ixf json --encoding "utf-8" "Path/to/IXF/file.IXF" "Path/to/OUTPUT/file.json"
    ```

=== "csv"

    ```bash
    db2ixf csv -e "utf-8" --sep "!" "Path/to/IXF/file.IXF" "Path/to/OUTPUT/file.csv"
    ```

=== "parquet"

    ```bash
    db2ixf parquet --encoding "utf-8"  --version "1.0" --batch-size 4000 "Path/to/IXF/file.IXF" "Path/to/OUTPUT/file.parquet"
    ```
